...
const instance = render(<UpdatePrompt current="0.4.1" latest="0.5.0" installer={scenario === "npm-env" ? "npm" : "Homebrew"} onUpdate={async (execute, _confirmTarget, onStage) => {
  const behavior = scenario === "interrupt" ? "setTimeout(()=>{},30000)"
    ...
    : scenario === "failure" ? "console.error('Error: checksum mismatch');process.exit(7)" : "process.exit(0)";
  ...
  await execute({ file: process.execPath, args: ["-e", child] }, { channel: scenario === "npm-env" ? "npm" : "homebrew", stage: "installing", current: "0.4.1", target: "0.5.0" });
